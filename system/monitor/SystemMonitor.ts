...
export class SystemMonitor implements ISystemMonitor {
  ...
  start(): void {
    ...
    this._logger?.sysLog("info", "SystemMonitor", `启动系统监控（间隔 ${this._updateInterval}ms）`);
    ...
    // 不阻止进程退出
    if (this._intervalTimer && typeof this._intervalTimer === "object" && "unref" in this._intervalTimer) {
