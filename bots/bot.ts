...
import {
    ...
    type C2MProcessTransportData,
    type M2CProcessTransportData,
...
async function main() {
    ...
    // 等待 IPC init（10s 超时）
        ...
        // IPC 超时
            ...
            if (config.botId || config.name) {
                console.warn(
                    `[bot] 警告: 未收到父进程配置，使用有限参数+默认值启动 (configured=false)`,
                );
            } else {
                // 无法启动
    ...
    // 命令调度器
    const dispatcher = new CommandDispatcher(context, (type) =>
        sendOutput('log', { message: `WARN-${type}`, level: 'warn' }),
    );
    ...
    const botManagerSendChat = (message: string) => {
