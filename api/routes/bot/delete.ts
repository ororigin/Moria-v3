    ...
    async (
      ...
      try {
        ...
        if (message.includes("仍在运行")) {
          return reply.status(400).send(error("BOT_ALREADY_RUNNING", `Bot ${botId} 仍在运行，请先停止或设置 force=true`));
        }
        return reply.status(500).send(error("DELETE_FAILED", `删除 Bot ${botId} 失败: ${message}`));
      }
    },
  );
}
