    ...
    async (
      ...
      if (!existingInfo) {
        ...
        if (!parsed.success) {
          ...
          return reply.status(400).send(error("INVALID_PARAMS", firstIssue?.message ?? "请求参数无效"));
        }
        const { name, server, port, password } = parsed.data;

        const configJson = JSON.stringify({
          name: name.trim(), server: server.trim(), port,
          ...(password ? { password } : {}),
        });
        ...
        try {
          ...
          try {
            ...
            return reply.status(201).send(success({ message: `Bot ${targetBotId} 配置已创建，但启动失败: ${detail}`, bot_id: targetBotId }));
          }

          return reply.status(201).send(success({ message: `Bot ${targetBotId} 已创建并启动`, bot_id: targetBotId }));
        } catch (err) {
      ...
      try {
        await botManager.startBot(targetBotId, force);
        return reply.status(200).send(success({ message: `Bot ${targetBotId} 启动中`, bot_id: targetBotId }));
      } catch (err) {
        const message = err instanceof Error ? err.message : String(err);
        if (message.includes("无法启动")) {
          return reply.status(400).send(error("BOT_ALREADY_RUNNING", `Bot ${targetBotId} 已在运行中，如需强制重启请设置 force=true`));
        }
        return reply.status(500).send(error("START_FAILED", `启动 Bot ${targetBotId} 失败: ${message}`));
      }
    },
  );
}
